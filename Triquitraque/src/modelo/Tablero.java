...
public class Tablero extends JPanel{
    ...
    public void cambiarEstilos(TipoImagen jugadorAct){
        if(jugadorAct==TipoImagen.CIRCULO){
            TableroJuego.imgJugadorEquis.setRuta(Ruta.JUGADORAUXILLAR);
            TableroJuego.imgJugadorEquis.repaint();
            TableroJuego.nombreJugadorEquis.setForeground(new Color(240,240,240,100));
            
            TableroJuego.imgJugadorCirculo.setRuta(Ruta.JUGADORCIRCULO);
            TableroJuego.imgJugadorCirculo.repaint();
            TableroJuego.nombreJugadorCirculo.setForeground(new Color(255,200,255));
        }else if(jugadorAct==TipoImagen.EQUIS){
            TableroJuego.imgJugadorCirculo.setRuta(Ruta.JUGADORAUXILLAR);
            TableroJuego.imgJugadorCirculo.repaint();
            TableroJuego.nombreJugadorCirculo.setForeground(new Color(240,240,240,100));
            
            TableroJuego.imgJugadorEquis.setRuta(Ruta.JUGADOREQUIS);
            TableroJuego.imgJugadorEquis.repaint();
            TableroJuego.nombreJugadorEquis.setForeground(new Color(180,232,255));
        }
    }
    public void resultado(TipoImagen tipoImagenResultado,TipoImagen jugadorGanador){
          ...
          if(tipoImagenResultado==TipoImagen.EMPATE){
              ...
              TimerTask tarea = new TimerTask() {
                  @Override
                  public void run() {
                    Resultado formResultado = new Resultado(TipoImagen.EMPATE,tablero);
              ...
              TimerTask tarea = new TimerTask() {
                  @Override
                  public void run() {
                      Resultado formResultado = new Resultado(jugadorGanador,tablero);
                       formResultado.setVisible(true);
                  }
              };
              timer.schedule(tarea,800);
    ...
    public void reiniciarTablero(TipoImagen ganador){
        ...
        if(ganador==TipoImagen.EQUIS){
            int puntajeNuevo= Integer.parseInt(TableroJuego.puntajeEquis.getText())+1;
            TableroJuego.puntajeEquis.setText(String.valueOf(puntajeNuevo));
        }
        else if(ganador==TipoImagen.CIRCULO){
             int puntajeNuevo= Integer.parseInt(TableroJuego.puntajeCirculo.getText())+1;
            TableroJuego.puntajeCirculo.setText(String.valueOf(puntajeNuevo));
        }
